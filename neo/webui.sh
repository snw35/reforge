...
printf "\n%s\n" "${delimiter}"
printf "\e[1m\e[32mInstall script for snw35/reforge:neo Container\n"
printf "\e[1m\e[34mTested on Ubuntu 24.04 LTS.\e[0m"
printf "\n%s\n" "${delimiter}"
...
# Container build already fetches versioned directory, no need to clone
printf "\n%s\n" "${delimiter}"
...
then
    printf "\e[1m\e[31mERROR: Nvidia GPU not detected, aborting...\e[0m"; exit 1;
fi
...
then
    printf "\n%s\n" "${delimiter}"
    printf "Check and activate python venv"
    printf "\n%s\n" "${delimiter}"
    cd "${install_dir}"/"${clone_dir}"/ || { printf "\e[1m\e[31mERROR: Can't cd to %s/%s/, aborting...\e[0m" "${install_dir}" "${clone_dir}"; exit 1; }
    if [[ ! -d "${venv_dir}" ]]
    then
        printf "\e[1m\e[31mERROR: Can't locate venv directory %s/, aborting...\e[0m" "${venv_dir}"; exit 1;
    fi
    # shellcheck source=/dev/null
